...
router.post("/", upload.fields([{name:"image"},{name:"musicaudio"}]),function(req, res){
    req.body.print.image = '/upload/'+ req.files["image"][0].filename;
    req.body.print.musicaudio = '/upload/'+ req.files["musicaudio"][0].filename;
...
router.post("/", middleware.isLoggedIn, upload.single('imageArtist'),function(req, res){
    req.body.print.imageArtist = '/upload/'+ req.file.filename;
    req.body.print.nameArtist = {
        id: req.user._id,
        username: req.user.username
    };
    Print.create(req.body.print, function(err, newlyAddedArtist){
        if(err){
            console.log(err);
        } else {
            res.redirect("/print/indexartist.ejs");
        }
    });
});
...
router.get("/newartist", middleware.isLoggedIn, function(req, res){
...
router.put('/:id',upload.fields([{name:"image"},{name:"musicaudio"}]), function(req, res){
    if(req.file["image"]){
        req.body.print.image = '/upload/' + req.files["image"][0].filename;
    }
    if(req.file["musicaudio"]){
        req.body.print.musicaudio = '/upload/' + req.files["musicaudio"][0].filename;
    }
